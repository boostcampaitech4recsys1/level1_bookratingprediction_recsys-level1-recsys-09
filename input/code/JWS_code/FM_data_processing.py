...
def data_processing(train,test,sub,book,user):
    ...
    # 인덱싱 처리된 데이터 조인
    context_df = ratings.merge(user, on='user_id', how='left').merge(book[['isbn','year_of_publication','category_high']], on='isbn', how='left')
    train_df = train.merge(user, on='user_id', how='left').merge(book[['isbn','year_of_publication','category_high']], on='isbn', how='left')
    test_df = test.merge(user, on='user_id', how='left').merge(book[['isbn','year_of_publication','category_high']], on='isbn', how='left')
    ...
    '''
    # 카테고리 인덱싱
    category_indexing = {v:k for k,v in enumerate(context_df['category_high'].unique())}
    train_df['category_high'] = train_df['category_high'].map(category_indexing)
    test_df['category_high'] = test_df['category_high'].map(category_indexing)
